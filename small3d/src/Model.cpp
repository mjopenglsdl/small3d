...
namespace small3d {
  ...
  void Model::loadIndexData() {
    ...
    for (auto face = facesVertexIndices.begin(); face != facesVertexIndices.end(); ++face) {
  ...
  void Model::correctDataVectors() {
    ...
    std::unique_ptr<std::unordered_map<int, int> > vertexUVPairs(new std::unordered_map<int, int>());
    ...
    for (int idx = 0; idx < numIndexes; ++idx) {
      ...
      for (int vertexIndex = 0; vertexIndex != 3; ++vertexIndex) {
        ...
        auto vertexUVPair = vertexUVPairs->find(facesVertexIndices[idx][vertexIndex]);
        ...
        if (line[0] == 'v' || line[0] == 'f') {
          ...
          if (line[0] == 'v' && line[1] == 'n') {
            ...
            for (int tokenIdx = 0; tokenIdx < numTokens; ++tokenIdx) {
              ...
              {
                if (t.find("//") != std::string::npos)   // normal index contained in the string
                {
                  v[idx - 1] = atoi(
                  ...
                  n.push_back(atoi(
                      t.substr(t.find("//") + 2).c_str()));
                         ...
                         && t.find("//") == std::string::npos)   // normal and texture coordinate index are
                  ...
                  for (int compIdx = 0; compIdx < numComponents; ++compIdx) {
                    ...
                    switch (componentIdx) {
                      ...
                      case 2:
                        ...
                        break;
                      default:
                        throw std::runtime_error("Unexpected component index number while parsing Wavefront file.");
                        break;
                    }
                    ++componentIdx;
                  }
