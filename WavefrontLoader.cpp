...
namespace small3d {
  ...
  void WavefrontLoader::loadVertexData()
  {
    ...
    model->vertexDataComponentCount = vertices->size() * 4;
    model->vertexDataSize = model->vertexDataComponentCount * sizeof(float);
	...
	for (int coordIdx = 0; coordIdx != 3; ++coordIdx)
	  {
	    model->vertexData[idx] = (*vertex)[coordIdx];
	    ++idx;
	  }
	model->vertexData[idx] = 1.0f;
	++idx;
      }
  }
  ...
  void WavefrontLoader::loadIndexData()
  {
    ...
    model->indexData = new unsigned int[numIndexes];
	...
	for (int indexIdx = 0; indexIdx != 3; ++indexIdx)
	  {
	    model->indexData[model->indexDataIndexCount] = (*face)[indexIdx] - 1; // -1 because Wavefront indexes
	    // are not 0 based
	    ++model->indexDataIndexCount;
	  }
  ...
  void WavefrontLoader::loadNormalsData()
  {
    ...
    if (model->vertexDataComponentCount == 0)
      {
	throw Exception(
    ...
    // vertex index)
    model->normalsDataComponentCount = 3 * vertices->size();
    model->normalsDataSize = model->normalsDataComponentCount * sizeof(float);

    model->normalsData = new float[model->normalsDataComponentCount];

    int faceVertexArrayIndex = 0;
    for(vector<int*>::iterator faceVertexIndex = facesVertexIndexes->begin(); faceVertexIndex != facesVertexIndexes->end(); ++faceVertexIndex)
      {
  ...
  void WavefrontLoader::loadTextureCoordsData()
  {
	...
	for(vector<int*>::iterator faceVertexIndex = facesVertexIndexes->begin(); faceVertexIndex != facesVertexIndexes->end();
	    ++faceVertexIndex)
	  {
	    for (int vertexIndex = 0; vertexIndex != 3; ++vertexIndex)
	      {
